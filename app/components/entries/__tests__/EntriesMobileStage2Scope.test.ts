...
describe('mobile entries stage 2 scope guard', () => {
  ...
  it('keeps mobile AI actions and register suggestions wired through the row editor', () => {
    expect(entriesPageSource).toContain(':register-ai-suggestions="registerAISuggestions"')
    expect(entriesPageSource).toContain('@ai-definition="(entry: any) => generateAIDefinition(entry)"')
    expect(entriesPageSource).toContain('@ai-examples="(entry: any) => generateAIExamples(entry)"')
    expect(entriesPageSource).toContain('@ai-categorize="(entry: any) => generateAICategorization(entry)"')
    ...
    expect(workbenchSource).toContain(':ai-loading-theme="isAILoadingForActive(\'theme\')"')
    expect(workbenchSource).toContain(':ai-loading-definition="isAILoadingForActive(\'definition\')"')
    expect(workbenchSource).toContain(':ai-loading-examples="isAILoadingForActive(\'examples\')"')
    ...
    expect(workbenchSource).toContain('@ai-categorize="activeEntry && $emit(\'ai-categorize\', activeEntry)"')
    expect(workbenchSource).toContain('@ai-definition="activeEntry && $emit(\'ai-definition\', activeEntry)"')
    expect(workbenchSource).toContain('@ai-examples="activeEntry && $emit(\'ai-examples\', activeEntry)"')
    expect(workbenchSource).toContain("props.aiLoadingFor?.entryKey === activeEntryKey.value")
    expect(rowEditorSource).toContain('AI 輔助')
    expect(rowEditorSource).toContain('AI 分類')
    expect(rowEditorSource).toContain('AI 釋義')
    expect(rowEditorSource).toContain('AI 例句')
    expect(rowEditorSource).toContain('AI 語域')
    expect(rowEditorSource).toContain('AI 語域建議')
    expect(rowEditorSource).toContain("'ai-categorize': []")
    expect(rowEditorSource).toContain("'ai-definition': []")
    expect(rowEditorSource).toContain("'ai-examples': []")
